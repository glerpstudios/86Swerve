...
public class SwerveDriveToPoseAroundNoGoZonesCommand extends Command {
    ...
    /**
     * The minimum distance to be held between the center of the robot and the no go
     * zones.
     * This is to ensure that the robot does not collide with the no go zones.
     * This also makes sure that, for no go zones with sharp corners, the robot can
     * make a rounded path around the corner.
     ...
     * It will be calculated using a pathfinding algorithm that avoids the no go
     * zones by the buffer distance.
     */
     ...
     * This is used to limit the acceleration of the robot when moving to the target
     * pose.
     */
     ...
     * This command will move the robot to the target pose while skirting the edges
     * of the no go zones in its path by the buffer distance.
     ...
     * @param bufferDistance                        The minimum distance to be held
     *                                              between the center of the robot
     *                                              and the no go zones, in meters.
     * @param targetPose                            The target pose for the robot to
     *                                              move to.
     * @param holonomicDriveController              The holonomic drive controller
     *                                              to use.
     * @param zones                                 The no go zones that the robot
     *                                              must avoid.
     * @param maxVelocityMetersPerSecond            The maximum velocity of the
     *                                              robot in meters per second.
     * @param maxAccelerationMetersPerSecondSquared The maximum acceleration of the
     *                                              robot in meters per second
     *                                              squared.
     */
    ...
    private double distancePointToSegment(Pose2d p, Pose2d v, Pose2d w) {
        ...
        if (l2 == 0)
            return Math.hypot(p.getX() - v.getX(), p.getY() - v.getY());
    ...
    /**
     * Returns the perpendicular distance from point p to the line defined by start
     * -> end.
     */
    ...
    public void initialize() {
        ...
        if (intersectingZones.isEmpty()) {
            ...
            for (int zoneIndex : intersectingZones) {
                ...
                ArrayList<Pose2d> intersections = getLinePolygonIntersections(currentPose, targetPose,
                        bufferedVertices);
                if (intersections.size() == 2) {
                    ...
                    // Step 4: pick only the waypoint with greatest perpendicular distance from line
    ...
    public void execute() {
        ...
        ChassisSpeeds chassisSpeeds = holonomicDriveController.calculate(currentPose, desiredState,
